...
module.exports.allFavouriteItem = async (req, res) => {
    try {
        ...
        const favouriteItems = favourites.map((favourite) => favourite.items).flat();
...
module.exports.deleteFavouriteItem = async (req, res) => {
    try {
        ...
        const updatedFavourite = favourite.items.filter((item) => item.productId != productId);

        if (!updatedFavourite) {
            return res.status(404).json({ message: 'favourite not found or item not found in the favourite' });
        } else {
